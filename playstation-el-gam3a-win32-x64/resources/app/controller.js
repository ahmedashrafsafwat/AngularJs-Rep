...
app.controller('myController', function($scope){
  ...
  //startTimer("clock", deadline, clientId);
    ...
    $scope.earningsOfTheDay = function() {
        console.log("I am called")
        var totalArr = []
        for (var i in $scope.clients) 
        {
            if($scope.clients[i].price != "NaN"){
            totalArr[i] = $scope.clients[i].price;
            totalArr[i] = totalArr[i].replace(" L.E", "");
            totalArr[i] = Number(totalArr[i]);

            console.log( "I am not equal to NaN");

            }else {
                totalArr[i] = 0;
                console.log("There is NaN here!!!")
            }

        }
        $scope.totalEarningsOfDay = totalArr.reduce(getSum);
        console.log("total earning of the day is " + $scope.totalEarningsOfDay)
    }
